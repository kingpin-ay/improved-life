...
const Login = () => {
    ...
    try {
      await axios.post('http://localhost:3000/users/auth', { 'username': username, 'password': password }).then((response) => {
        if(response.data.isPass){
          setAuth({ username: response.data['username'], id: response.data['id'] })
          setLoggedAs(response.data['username'])
        }
        else{
          alert("login faild please check ID and Password")
        }
  ...
  return (
    <>
      {loggedAs ? (
        <>
          <div className="d-flex aligh-items-center p-4 mt-5 flex-column text-center">
            <h2>Welcome back ! {loggedAs}</h2>
          </div>
        </>
      ) : (
        <>
